...
def is_valid_token_obj(request, token_obj):
    ...
    if (
        ...
        and token_obj.activated == True
        and token_obj.invalidated == False
    ):
        return True
...
def votings(request):
    ...
    if is_valid_token(request) == False:
        return JsonRespose(
            "message",
            "Could not return voting list due to non-eligible token.",
            status=401,
        )

    closed_votings = list(
        Voting.objects.filter(is_open=False, is_ended=False).order_by("-id")
    )
    open_votings = []
    ended_votings = list(
        Voting.objects.filter(is_open=False, is_ended=True).order_by("-id")
    )
...
def user_status(request):
    ...
    if is_valid_token(request) == True:
        return JsonResponse(
            {
                "status": 1,
                "token": get_token_obj(request).token,
    ...
    return JsonResponse(
        {
            "status": 0,
            "token": "",
            "message": "Token does not exist, is not active or has been invalidated.",
        },
        status=200,
    )
...
def voting_results(request):
    ...
    return render(request, "voting_results.html", {"votings": votings})
...
def admin_voting_list(request):
    ...
    closed_votings = Voting.objects.filter(is_open=False, is_ended=False).order_by(
        "-id"
    )
    ...
    open_votings_eligible_token_counts = list(
        map(
            lambda x: (x[0], len(x[1])),
            get_eligible_active_tokens(request, open_votings),
        )
    )
